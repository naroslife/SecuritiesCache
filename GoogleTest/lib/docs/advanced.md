...
is awkward especially when the expression has _side-effects or is expensive to
evaluate.
...
Since `statement` runs in the child process, any in-memory _side effect (e.g.
